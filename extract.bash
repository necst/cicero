...
for i in $(ls build_$j | grep u96_mesh_16)
#for i in 16
do
	dsgn_name=u96_mesh_${i//u96_mesh_/}
	prj_name=u96_mesh_${i//u96_mesh_/}
	top_name=cicero_top_mesh_${i//u96_mesh_/}
...
for i in $(ls build_$j | grep u96_ring_1)
#for i in 10
do
	...
	vivado -mode batch -source extraction.tcl -tclargs $(pwd)/build_$j/${dsgn_name} ${i//u96_ring_/} $(pwd)/build_$j ring
	vivado -mode batch -source extract.tcl -tclargs $(pwd)/build_$j/${dsgn_name} ${i//u96_ring_/} $(pwd)/build_$j ring
